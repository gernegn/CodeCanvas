...
class AdminController extends Controller
{
    ...
    public function userCode(Request $request)
    {
        ...
        // ✅ ใช้ leftJoin ดึงชื่อ User_Name จากตาราง ccv_UserGeneral มาเชื่อมกับ UserCode
        $allData = UserCode::leftJoin('ccv_UserGeneral', 'ccv_UserCode.User_ID', '=', 'ccv_UserGeneral.User_ID')
            ->select('ccv_UserCode.*', 'ccv_UserGeneral.User_Name')
            ->orderBy('ccv_UserCode.created_at', $sort)
            ->get();
        ...
        if ($search) {
            ...
            if (is_numeric($search)) {
                ...
                $userCodes = $allData->filter(function ($item) use ($search) {
                    // ✅ เพิ่มให้ค้นหาโดยพิมพ์ "ชื่อผู้เล่น" ได้ด้วย (ใช้ stripos เพื่อค้นหาแบบไม่สนพิมพ์เล็ก-ใหญ่)
                    return stripos($item->User_Code, $search) !== false
                        || stripos($item->Color_Name, $search) !== false
                        || stripos($item->Texture_Name, $search) !== false
